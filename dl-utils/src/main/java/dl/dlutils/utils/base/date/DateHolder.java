...
public class DateHolder implements Serializable, Cloneable, Comparable<DateHolder> {
	...
	/**
	 * Initializes calendar with current date and uses the time zone and the
	 * locale of the ContextUser if exists.
	 * 
	 * @see DateUtils#getCalendar()
	 */
	public DateHolder() {
	 ...
	 * @param precision
	 * @param locale
	 * @see DateUtils#getCalendar(Locale)
	 */
	public DateHolder(final DatePrecision precision, final Locale locale) {
		calendar = DateUtils.getCalendar(locale);
		setPrecision(precision);
	}
	...
	public DateHolder(final Date date, final DatePrecision precision, final Locale locale) {
		this.calendar = DateUtils.getCalendar(locale);
		this.calendar.setTime(date);
		setPrecision(precision);
	}
	...
	/**
	 * Initializes calendar with given date and uses the given time zone and the
	 * locale of the ContextUser if exists.
	 */
	...
	/**
	 * Initializes calendar with current date and uses the given time zone and
	 * the locale of the ContextUser if exists.
	 * 
	 * @see DateUtils#getCalendar(TimeZone)
	 */
	...
	/**
	 * Date will be set. Dependent on precision, also seconds etc. will be set
	 * to zero. Ensures the precision.
	 */
	...
	/**
	 * Sets the precision of the date represented by this object. Ensures the
	 * precision.
	 * 
	...
	/**
	 * Ensure the given precision by setting / rounding fields such as minutes
	 * and seconds. If precision is MINUTE_15 then rounding the minutes down:
	 * 00-14 -&gt; 00; 15-29 -&gt; 15, 30-44 -&gt; 30, 45-59 -&gt; 45.
	 */
	...
	/**
	 * Considers the time zone of the user, for example, if date is 20.11.1970
	 * 23:00:00 UTC but the user's locale is Europe/Berlin then the
	 * java.sql.Date should be 21.11.1970! <br/> This methods transforms first
	 * the day into UTC and then into java.sql.Date.
	 */
	
	public java.sql.Date getSQLDate() {
		final Calendar cal = Calendar.getInstance(DateUtils.UTC);
	...
	/**
	 * Has the given date the same day? The given date will be converted into a
	 * calendar (clone from this) with same time zone.
	 * 
	 * @param date
	 * @return
	 */
	...
	/**
	 * Sets the date to the beginning of the year (first day of year) and calls
	 * setBeginOfDay.
	 * 
	 * @see #setBeginOfDay()
	 */
	...
	/**
	 * Sets the date to the beginning of the month (first day of month) and
	 * calls setBeginOfDay.
	 * 
	 * @see #setBeginOfDay()
	 */
	...
	/**
	 * Sets the date to the beginning of the week (first day of week) and calls
	 * setBeginOfDay.
	 * 
	 * @see #setBeginOfDay()
	 */
	...
	/**
	 * Checks on day equals first day of week and hour, minute, second and
	 * millisecond equals zero.
	 */
	...
	/**
	 * Sets the date to the ending of the week (last day of week) and calls
	 * setEndOfDay.
	 * 
	 * @see #setEndOfDay()
	 */
	...
	/**
	 * Setting the date from the given object (only year, month, day). Hours,
	 * minutes, seconds etc. will be preserved.
	 * 
	...
	/**
	 * Gets the time of day in milliseconds since midnight. This method is used
	...
	/**
	 * Compute time of all fields by calling by calling
	 * calendar.getTimeInMillis. Don't forget to call ensurePrecision if needed.
	 * 
	 * @see #ensurePrecision()
	...
	/**
	 * @return
	 * @see DateUtils#getWeekOfYear(Date)
	 */
	public int getWeekOfYear() {
		return DateUtils.getWeekOfYear(calendar);
	}
	...
	public String toString() {
		return DateUtils.formatAsUTC(getDate()) + ", time zone=" + calendar.getTimeZone().getID() + ", date=" + getDate().toString();
	}
	...
	/**
	 * Sets the date by giving all datefields and compute all fields. Set
	 * millisecond to zero.
	 * 
	 * @param year
	 * @param month
	 ...
	 * @see #ensurePrecision()
	 */
	public DateHolder setDate(final int year, final int month, final int date, final int hourOfDay, final int minute, final int second, final int millisecond) {
