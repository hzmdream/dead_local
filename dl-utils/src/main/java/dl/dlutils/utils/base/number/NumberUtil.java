...
package dl.dlutils.utils.base.number;
...
public class NumberUtil {
	...
	public static String formatBytes(final long bytes) {
		...
		if (bytes < MEGA_BYTES) {
			...
			return NumberFormat.getInstance(Locale.getDefault()).format(no) + " kb";
		}
		if (bytes < GIGA_BYTES) {
			...
			return NumberFormat.getInstance(Locale.getDefault()).format(no) + " Mb";
		}
	 ...
	 * 00##. Example: +49 561 / 316793-0 -> 00495613167930 <br/> Ignores any
	 * characters after the first occurence of ':' or any letter.
	 * 
	 * @param str
	 * @param countryPrefix If country prefix is given, for all numbers
	 *            beginning with the country prefix the country prefix will be
	 *            replaced by 0. Example: ("+49 561 / 316793-0", "+49") ->
	 *            05613167930; ("+39 123456", "+49") -> 0039123456.
	 * @return
	 */
	...
	public static BigDecimal setDefaultScale(final BigDecimal number) {
		...
		if (number.compareTo(NumberUtil.HUNDRED) >= 0 || number.compareTo(NumberUtil.MINUS_HUNDRED) <= 0) {
			return number.setScale(0, BigDecimal.ROUND_HALF_UP);
		} else if (number.compareTo(NumberUtil.TWENTY) >= 0 || number.compareTo(NumberUtil.MINUS_TWENTY) <= 0) {
